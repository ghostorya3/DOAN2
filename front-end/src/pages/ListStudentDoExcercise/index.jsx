...
import { GET, POST } from "../../components/common";
...
export default function ListStudentDoExcercise() {
  ...
  const { data } = useQuery({
    queryKey: ["getListStudentDoExcercise"],
    queryFn: getListStudentDoExcercise,
  });
  ...
  const handleExportFile = async () => {
    const res = await POST("/class/getListStudentDoExcercise", {
      id,
      status,
      isExport: true,
    }); 
    if (res.result) {
     window.open(`${import.meta.env.VITE_URL_API}/download?file=${res.url}`, "_blank");
    }
  };
  return (
    <>
      <div className="p-5 w-full">
        <div
          className="p-3 border rounded-full bg-black w-12 h-12 flex justify-center items-center cursor-pointer"
          onClick={() => navigate(-1)}
        >
        ...
        <div className="w-full flex flex-col justify-center text-3xl font-semibold items-center">
          ...
          <div className="text-xl flex gap-5 justify-between w-full px-10">
            <div className="text-xl flex flex-col gap-5 justify-start">
              <div>
                Tên bài tập:{" "}
                <span className="font-normal">{data?.dataWork?.name}</span>
              </div>
              <div>
                Mã bài tập:{" "}
                <span className="font-normal">{data?.dataWork?._id}</span>
              </div>
            </div>
            <div className="font-normal text-base flex items-end gap-3">
              ...
              <div
                className="border rounded-xl py-1 px-2 cursor-pointer mt-2 bg-green-500 text-white font-medium"
